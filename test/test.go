...
func runConn(wg *sync.WaitGroup, netowrk, remote string, timeout time.Duration) {
	...
	go func() {
		...
		var err error
		for running {
			writed, err = fmt.Fprintf(conn, "send-send-send-send-send-send-send-send-send-sendsend-send-send-send-send-send-send-send-send-sendsend-send-send-send-send-send-send-send-send-sendsend-send-send-send-send-send-send-send-send-send->%v", i)
			if err != nil {
				break
			}
